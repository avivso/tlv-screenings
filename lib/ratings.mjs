...
/**
 * Identify a film with TMDB (which handles Hebrew titles) and return its IMDb
 * id plus English title, year, poster and TMDB score. The IMDb score itself is
 * added later from IMDb's own dataset (see imdb.mjs). Results are cached.
 */
export async function getRatings(film, { env, cache, log = () => {} }) {
  ...
  try {
    ...
    return cached?.data ?? { query: cleanTitle(film.title), imdbId: null, match: "none" };
  }
...
async function lookup(film, env) {
  ...
  if (override) {
    ...
    const found = await getJson(`${TMDB}/find/${override}?api_key=${env.TMDB_API_KEY}&external_source=imdb_id`);
    const hit = found?.movie_results?.[0];
    if (hit) Object.assign(result, details(hit));
    return result;
  }
  ...
  return Object.assign(result, details(det || hit.movie), {
...
const details = (m) => ({
  ...
  tmdbRating: m.vote_count >= 20 ? round1(m.vote_average) : null,
