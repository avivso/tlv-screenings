...
export async function scrapeMovieland(_ctx, { today, days, dbg, log }) {
  ...
  try {
    ...
    // edge case. seret.co.il lists the same cinema and serves plain HTML — but
    // it is behind the same block, so it only helps from an unblocked IP.
    dbg.apiError = e.message;
    log(`movieland: own site unavailable (${e.message}); falling back to seret.co.il`);
    let rows = [];
    try {
      rows = await scrapeSeretTheater(SERET_TID, THEATER, { today, days, dbg, log });
      dbg.source = "seret";
    } catch (fallbackError) {
      dbg.seretError = fallbackError.message;
      log(`movieland: seret.co.il also unavailable (${fallbackError.message})`);
    }
    if (!rows.length) {
      throw new Error(
        "Its listings can't be refreshed from the server — movieland.co.il and the Israeli " +
        "listing sites all refuse it. Showing the last listings that came through."
      );
    }
    return rows;
  }
