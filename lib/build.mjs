...
export async function buildListings({ env, cache, previous = null, days = 7, log = console.log }) {
  ...
  if (env.TMDB_API_KEY) {
    ...
    errors.push({ venue: null, message: "No TMDB_API_KEY set, so films can't be identified and ratings are missing." });
  }

  // IMDb's own dump needs no key and is refreshed daily.
  ...
  for (const f of list) {
    const r = f.ratings || {};
    out[f.key] = {
      // Once TMDB has matched the film, its canonical title beats the venue's.
      titleEn: r.titleEn || f.titleEn || null,
      year: f.year || r.year || null,
      imdbId: r.imdbId || null,
      match: r.match || "none",
      imdbRating: imdbScores.get(r.imdbId)?.imdbRating ?? null,
      imdbVotes: imdbScores.get(r.imdbId)?.imdbVotes ?? null,
