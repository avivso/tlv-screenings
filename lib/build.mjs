...
export async function buildListings({ env, cache, previous = null, days = 7, log = console.log }) {
  ...
  if (previous?.screenings) {
    for (const venue of new Set(errors.map((e) => e.venue).filter(Boolean))) {
      ...
      const carried = previous.screenings.filter((s) => s.venue === venue && allDays.includes(s.date));
  ...
  if (env.OMDB_API_KEY || env.TMDB_API_KEY) {
    ...
    if (omdbError) {
      errors.push({
        venue: null,
        message: `OMDb is not answering (${omdbError}), so IMDb, Rotten Tomatoes and Metacritic scores are missing.`,
      });
    }
  } else {
  ...
  return {
    listings: {
      ...
      screenings: screenings.map(({ date, time, venue, title, titleEn, year, runtime, ticketUrl, infoUrl, retro, filmKey, stale }) => ({
        date, time, venue, title, titleEn, year, runtime, ticketUrl, infoUrl, retro, filmKey, stale: stale || undefined,
      })),
      films: out,
      errors,
    },
